...
public class ItemBucketSlime extends ItemBetterStorage {
	...
	@Override
	@SideOnly(Side.CLIENT)
	public Icon getIconIndex(ItemStack stack) { return getIcon(stack, 0); }
		...
		if ((effectList != null) && (handler != null)) {
			...
			for (int i = 0; i < Math.min(effectList.tagCount(), max); i++) {
				PotionEffect effect = PotionEffect.readCustomPotionEffectFromNBT(
						(NBTTagCompound)effectList.tagAt(i));
				...
				for (int i = 0; i < effectList.tagCount(); i++) {
					PotionEffect effect = new PotionEffect(
							PotionEffect.readCustomPotionEffectFromNBT(
										(NBTTagCompound)effectList.tagAt(i)));
					effect.duration = (int)(effect.getDuration() * handler.durationMultiplier());
